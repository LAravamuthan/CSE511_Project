...
The hot zone analysis takes as input two CSV files. One CSV file is the taxi trips CSV which will be used to extract the pickup points and the
second CSV is the zones CSV file for the rectangles for which the hot zone analysis has to be done.

The basic workflow for hot zone analysis is as follows:
1. Parse the input CSV files to load point data.
2. Parse the input CSV files to load rectangle data.
3. Join the point data and rectangle data using ST_CONTAINS to get all points which lie within the extracted rectangles. Please note that
we have used the ST_CONTAINS from phase 1 for this purpose.
4. Run a count aggregate function on the join result to count the number of points found within each rectangle. We have grouped by the rectangle
column since the join result will have multiple point entries for the same rectangle data and we need to count these repeating rectangles. Finally, we have ordered
by the rectangle column to follow the ordering expected in the problem statement.

We were able to successfully run the code on our local Spark server and verify the sample output for the sample input. The code runs successfully for the larger
trips dataset as well.
